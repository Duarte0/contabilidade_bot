...
class SQLiteManager:
    ...
    def get_contas_fixas_cliente(self, cliente_id: int) -> List[ContaFixa]:
        """Busca todas as contas fixas do cliente (renomeado para evitar conflito)"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def get_ultimo_pagamento_cliente(self, cliente_id: int) -> Optional[str]:
        """Busca a data do último pagamento do cliente"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def get_cliente_by_id(self, cliente_id: int) -> Optional[Cliente]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT id, nome, digisac_contact_id, telefone, email FROM clientes WHERE id = ?', (cliente_id,))
            result = cursor.fetchone()
            return Cliente(*result) if result else None